...
export const getFeatureImages = createAsyncThunk('api/common/features', async () => {
    const response = await axios.get(`http://localhost:4000/api/common/features/get`);
    console.log(response.data);
    
    return response.data;
});
export const addFeatureImages = createAsyncThunk('api/common/features', async (image) => {
    const response = await axios.post(`http://localhost:4000/api/common/features/add`, { image });
    return response.data;
});
